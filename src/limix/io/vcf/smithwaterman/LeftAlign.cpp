...
bool leftAlign(string& querySequence, string& cigar, string& baseReferenceSequence, int& offset, bool debug) {
    ...
    for (vector<IndelAllele>::iterator id = indels.begin(); id != indels.end(); ++id) {
        ...
        // the repeat unit.
        //
        unsigned int steppos, readsteppos;
	...
	if (!del->insertion) {
	    ...
	    unsigned int minsize = indel.length;
	    unsigned int flankingLength = indel.readPosition;
	    if (debug) cerr << indel << endl;
	    ...
	    if (minsize >= 0 && minsize < (unsigned int) indel.length) {
		...
		unsigned int flankingLength = querySequence.size() - indel.readPosition + indel.readLength();
		...
		if (minsize >= 0 && minsize <= indel.length) {
			...
			if (flankingLength < (unsigned int) softEnd.size()) {
    ...
    */
	...
	if ((id + 1) == newIndels.end()
	    && (indel.insertion && (unsigned int) indel.position == (unsigned int) referenceSequence.size()
		|| (!indel.insertion && (unsigned int) indel.position + (unsigned int) indel.length == (unsigned int) referenceSequence.size()))
	    ) {
	    if (indel.insertion) {
