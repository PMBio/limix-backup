/*
 * NewCVarianceDecomposition.cpp
 *
 *  Created on: Nov 27, 2012
 *      Author: stegle
 */

#include "NewCVarianceDecomposition.h"
...
muint_t AVarianceTermN::getNumberIndividuals() const throw(CGPMixException)
{
	if (K_mode==-1)
...
void AVarianceTermN::setK(const MatrixXd& K) throw(CGPMixException)
{
	if(K.rows()!=K.cols())
		throw CGPMixException("AVarianceTermN: K needs to be a squared matrix!");
	this->K = K/K.diagonal().mean();
	Kcf = PTFixedNew(new CTFixedNew(K.rows(),this->K));
	this->K_mode = 0;
}

void AVarianceTermN::setX(const MatrixXd& X) throw(CGPMixException)
{
	this->X = X;
...
void AVarianceTermN::agetK(MatrixXd *out) const throw(CGPMixException)
{
	(*out) = this->K;
}

void AVarianceTermN::agetX(MatrixXd *out) const throw(CGPMixException)
{
	(*out) = this->X;
}
...
CSingleTraitTerm::CSingleTraitTerm():AVarianceTermN() {
}

CSingleTraitTerm::CSingleTraitTerm(const MatrixXd& K):AVarianceTermN() {
	this->setK(K);
}
...
void CSingleTraitTerm::agetScales(VectorXd* out) const throw(CGPMixException)
{
	if (K_mode==-1)
		throw CGPMixException("CSingleTraitTerm: K needs to be set!");
	static_pointer_cast<CTraitNew>(this->Kcf)->agetScales(out);
}
...
CMultiTraitTerm::CMultiTraitTerm(muint_t P):AVarianceTermN()
{
	this->P=P;
	this->TCtype="Null";
}

CMultiTraitTerm::CMultiTraitTerm(muint_t P, std::string TCtype, const MatrixXd& K):AVarianceTermN()
{
	this->P=P;
...
CMultiTraitTerm::CMultiTraitTerm(muint_t P, std::string TCtype, muint_t p, const MatrixXd& K):AVarianceTermN()
{
	this->P=P;
...
{
	if(TCtype=="FreeForm")			traitCovariance = PTFreeFormNew(new CTFreeFormNew(this->P));
	else if (TCtype=="Block")		traitCovariance = PTFixedNew(new CTFixedNew(this->P,MatrixXd::Ones(P,P)));
	else if (TCtype=="Identity")	traitCovariance = PTFixedNew(new CTFixedNew(this->P,MatrixXd::Identity(P,P)));
	else if (TCtype=="Dense")		traitCovariance = PTDenseNew(new CTDenseNew(this->P));
	else if (TCtype=="Diagonal")	traitCovariance = PTDiagonalNew(new CTDiagonalNew(this->P));
	else if (TCtype=="Full")		traitCovariance = PTLowRankNew(new CTLowRankNew(this->P));
		...
		K0(p,p) = 1;
		traitCovariance = PTFixedNew(new CTFixedNew(this->P,K0));
...
CNewVarianceDecomposition::CNewVarianceDecomposition(const MatrixXd& pheno){
	this->setPheno(pheno);
}

CNewVarianceDecomposition::~CNewVarianceDecomposition(){
}

void CNewVarianceDecomposition::clear()
{
...
void CNewVarianceDecomposition::addFixedEffTerm(const std::string fixedType) throw(CGPMixException)
{
	MatrixXd singleTraitFixed = MatrixXd::Ones(this->N,1);
	this->addFixedEffTerm(fixedType,singleTraitFixed);
}

void CNewVarianceDecomposition::addFixedEffTerm(const std::string fixedType, const MatrixXd& singleTraitFixed) throw(CGPMixException)
{
	if ((muint_t)singleTraitFixed.cols()!=(muint_t)1 || (muint_t)singleTraitFixed.rows()!=this->N)
		throw CGPMixException("CNewVarianceDecomposition: the single trait fixed effect has incompatible shape");
	if (fixedType=="common") {
		...
		throw CGPMixException("CNewVarianceDecomposition: non valid fixed effect type");
}

void CNewVarianceDecomposition::addFixedEffTerm(const MatrixXd& fixed) throw(CGPMixException)
{
	if (fixed.cols()!=this->P || fixed.rows()!=this->N)
		throw CGPMixException("CNewVarianceDecomposition: the fixed effect has incompatible shape");
	fixedEffs.push_back(fixed);
}

void CNewVarianceDecomposition::getFixedEffTerm(MatrixXd *out, const muint_t i) const throw(CGPMixException)
{
	if (i>=this->getNumberFixedEffs())
		throw CGPMixException("CNewVarianceDecomposition: value out of range");
	(*out)=this->fixedEffs[i];
}

void CNewVarianceDecomposition::clearFixedEffs()
{
	this->fixedEffs.clear();
}

muint_t CNewVarianceDecomposition::getNumberFixedEffs() const
{
	return (muint_t)(this->fixedEffs.size());
}

void CNewVarianceDecomposition::setPheno(const MatrixXd& pheno) throw(CGPMixException)
{
...
void CNewVarianceDecomposition::getPheno(MatrixXd *out) const throw(CGPMixException)
{
	(*out)=this->pheno;
}

void CNewVarianceDecomposition::addTerm(PVarianceTermN term) throw(CGPMixException)
{

	if (term->getName()=="CMultiTraitTerm")
		if (term->getNumberTraits()!=this->P)
			throw CGPMixException("CNewVarianceDecomposition: the term has incompatible number of traits");
		if (term->getNumberIndividuals()!=this->N)
			throw CGPMixException("CNewVarianceDecomposition: the term has incompatible number of individual");
	else if (term->getName()=="CSingleTraitTerm")
		if (term->getNumberIndividuals()!=this->N*this->P)
			throw CGPMixException("CNewVarianceDecomposition: the single trait term must have dimensions NP");
	terms.push_back(term);
}

void CNewVarianceDecomposition::addTerm(const MatrixXd& K) throw(CGPMixException)
{
//TODO
}

void CNewVarianceDecomposition::addTerm(std::string TCtype, const MatrixXd& K) throw(CGPMixException)
{
	this->addTerm(TCtype,(muint_t)0,K);
}

void CNewVarianceDecomposition::addTerm(std::string TCtype, muint_t p, const MatrixXd& K) throw(CGPMixException)
{
...
PVarianceTermN CNewVarianceDecomposition::getTerm(muint_t i) const throw(CGPMixException)
{
...
void CNewVarianceDecomposition::clearTerms()
{
	this->terms.clear();
}

muint_t CNewVarianceDecomposition::getNumberTerms() const
{
	return (muint_t)(terms.size());
}

void CNewVarianceDecomposition::setScales(const VectorXd& scales) const throw(CGPMixException)
{
	if (this->is_init==0)
		throw CGPMixException("CNewVarianceDecomposition: CNewVarianceDecomposition needs to be initialised");
	this->covar->setParams(scales);
}

void CNewVarianceDecomposition::setScales(muint_t i,const VectorXd& scales) const throw(CGPMixException)
{
	if (i>=this->getNumberTerms())
		throw CGPMixException("CNewVarianceDecomposition: value out of range");
...
void CNewVarianceDecomposition::agetScales(muint_t i, VectorXd* out) const throw(CGPMixException)
{
	if (i>=this->getNumberTerms())
		throw CGPMixException("CNewVarianceDecomposition: value out of range");
...
void CNewVarianceDecomposition::agetScales(VectorXd* out) throw(CGPMixException)
{
	(*out).resize(this->getNumberScales(),1);
	muint_t row=0;
	for(PVarianceTermNVec::iterator iter = this->terms.begin(); iter!=this->terms.end();iter++)
	{
		PVarianceTermN term = iter[0];
		VectorXd scales;
...
muint_t CNewVarianceDecomposition::getNumberScales() throw(CGPMixException)
{
	muint_t out=0;
	for(PVarianceTermNVec::iterator iter = this->terms.begin(); iter!=this->terms.end();iter++)
	{
...
void CNewVarianceDecomposition::agetTraitCovariance(muint_t i, MatrixXd *out) const throw(CGPMixException)
{
	if (i>=this->getNumberTerms())
		throw CGPMixException("CNewVarianceDecomposition: value out of range");
...
void CNewVarianceDecomposition::agetVariances(muint_t i, VectorXd* out) const throw(CGPMixException)
{
	if (i>=this->getNumberTerms())
		throw CGPMixException("CNewVarianceDecomposition: value out of range");
...
void CNewVarianceDecomposition::agetVariances(MatrixXd* out) throw(CGPMixException)
{
...
void CNewVarianceDecomposition::agetVarComponents(MatrixXd* out) throw(CGPMixException)
{
	agetVariances(out);
...
void CNewVarianceDecomposition::initGP() throw(CGPMixException)
{
...
void CNewVarianceDecomposition::getFixedEffects(VectorXd* out) throw(CGPMixException)
{
	(*out)=this->gp->getParams()["dataTerm"];
}

mfloat_t CNewVarianceDecomposition::getLML() throw(CGPMixException)
{
	if (!this->is_init)
		throw CGPMixException("CNewVarianceDecomposition: the term is not initialised!");
	return -1.*this->gp->LML();
}

mfloat_t CNewVarianceDecomposition::getLMLgrad() throw(CGPMixException)
{
	if (!this->is_init)
		throw CGPMixException("CNewVarianceDecomposition: the term is not initialised!");
	mfloat_t out = 0;
