...
class BLUP(object):
    ...
    def fit(self, XTrain=None, yTrain=None, KTrain=None, delta=None, folds=0):
        ...
        if delta == None:
            if folds > 0: #initialize cross-learning of delta
                self.ldelta = self.crossvalidate_delta(folds)
            else:
                self.ldelta = lmm.optdelta(self.Uy[:,0],self.Uone,self.S, ldeltamin=-5.0, ldeltamax=5.0, numInt=100000)
            #print 'ldelta of BLUP is:', self.ldelta
    ...
    def update_delta(self, mean):
        ...
        self.ldelta = lmm.optdelta(self.Uy[:,0],Umean,self.S, ldeltamin=self.ldelta-1.0, ldeltamax=self.delta+1)
        self.delta = SP.exp(self.ldelta)
        return self.delta
    ...
    def crossvalidate_delta(self, folds):
        ...
        for ldelta in ldeltas:
            for test_set in cv_scheme:
                ...
                Core = SP.dot(K_cross, LA.inv((K_sub + SP.eye(train_set.sum())*SP.exp(ldelta))))
                diff =  self.yTrain[test_set] - SP.dot(Core, self.yTrain[train_set])
